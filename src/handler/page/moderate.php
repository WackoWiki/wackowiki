...
if (($this->is_moderator() && $this->has_access('read')) || $this->is_admin())
{
	...
	else
	{
		...
		// make collector query
		$sql =
			'SELECT p.page_id, p.owner_id, p.user_id, p.tag, p.title, p.user_id, p.owner_id, ip, SUBSTR(body, 1, 500) AS body, p.created, p.page_lang, u.user_name, o.user_name as owner_name ' .
			'FROM ' . $prefix . 'page p ' .
				'LEFT JOIN ' . $prefix . 'user u ON (p.user_id = u.user_id) ' .
				'LEFT JOIN ' . $prefix . 'user o ON (p.owner_id = o.user_id) ' .
			$selector .
