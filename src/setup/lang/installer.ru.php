...
/*
   Generic Page Text
*/
'TitleInstallation'				=> 'Установка WackoWiki',
'TitleUpdate'					=> 'Обновление WackoWiki',
'Continue'						=> 'Продолжить',
'Back'							=> 'Назад',
...
/*
   Locking Check
 */
'LockAuthorization'				=> 'Авторизация',
'LockAuthorizationInfo'			=> 'Пожалуйста, введите пароль, который вы сохранили в файле %1.',
'LockPassword'					=> 'Пароль:',
'LockLogin'						=> 'Логин',
'LockPasswordInvalid'			=> 'Неверный пароль.',
'LockedTryLater'				=> 'Этот сайт в настоящее время обновляется. Пожалуйста, повторите попытку позже.',
'EmptyAuthFile'					=> 'Отсутствует или пустой файл %1 . Пожалуйста, создайте файл и установите пароль в этот файл.',
...
*/
'lang'							=> 'Выбор языка',
'PleaseUpgradeToR6'				=> 'Вы используете доисторическую версию WackoWiki %1. Для обновления до текущей версии сперва необходим отдельный апгрейд до %2.',
...
'ErrorMinPhpVersion'			=> 'Версия PHP должна быть больше, чем %1. Ваш сервер запущен более ранней версии. Для корректной работы WackoWiki необходимо обновить PHP до более свежей версии.',
...
'AclMode'						=> 'Настройки ACL по умолчанию',
'AclModeDesc'					=> '',
'PublicWiki'					=> 'Публичная вики (чтение для всех, писать и комментировать для зарегистрированных пользователей)',
'PrivateWiki'					=> 'Частная вики (чтение, запись, комментарий только для зарегистрированных пользователей)',
...
'Base'							=> 'Базовый URL',
'BaseDesc'						=> 'Базовый URL сайта WackoWiki. К нему будут добавляться имена страниц; если используется mod_rewrite, адрес должен оканчиваться прямой косой чертой, т. е.',
...
'DbSqlMode'						=> 'Режим SQL',
'DbSqlModeDesc'					=> 'Режим SQL, который вы хотите использовать.',
'DbVendor'						=> 'Марка СУБД',
'DbVendorDesc'					=> 'Марка используемой вами СУБД.',
'DbCharset'						=> 'Кодировка',
...
'DbNameSqliteDesc'				=> 'Каталог данных и имя файла SQLite должны использоваться для WackoWiki.',
'DbNameSqliteHelp'				=> 'SQLite хранит все данные в одном файле.<br><br>Каталог должен быть доступен для записи веб-сервером во время установки.<br><br>Она должна быть <strong>не</strong> доступна через Интернет.<br><br>Программа установки напишет <code>. taccess</code> файл вместе с ним, но если кто-то не сможет получить доступ к вашей сырой базе данных.<br>Это включает в себя необработанные данные (адреса электронной почты, хэшированные пароли), а также защищенные страницы и другие ограниченные данные на wiki.<br><br>Рассмотрим установку базы данных где-нибудь в целом, например в <code>/var/lib/wackowiki/yourwiki</code>.',
...
'SqliteFileExtensionError'		=> 'Пожалуйста, используйте одно из следующих расширений файла db, sdb, sqlite.',
'SqliteParentUnwritableGroup'	=> 'Не удается создать каталог данных <code>%1</code>, потому что родительский каталог <code>%2</code> не доступен для записи на веб-сервер.<br><br>Программа установки определила пользователя, под которым работает веб-сервер.<br>Сделайте <code>%3</code> каталог доступным для записи для продолжения.<br>На системе Unix/Linux делает:<br><br><pre>cd %2<br>mkdir %3<br>chgrp %4 %3%3<br>chmod g+w %3</pre>',
'SqliteParentUnwritableNogroup'	=> 'Не удается создать каталог данных <code>%1</code>, потому что родительский каталог <code>%2</code> не доступен для записи на веб-сервер.<br><br>Программа установки не может определить пользователя, под которым работает веб-сервер.<br>Сделайте директорию <code>%3</code> глобально доступной для записи в нее (и другие!), чтобы продолжить.<br>На системе Unix/Linux делает:<br><br><pre>cd %2<br>mkdir %3<br>chmod a+w %3</pre>',
'SqliteMkdirError'				=> 'Ошибка создания каталога данных <code>%1</code>.<br>Проверьте местоположение и повторите попытку.',
'SqliteDirUnwritable'			=> 'Не удается записать в директорию <code>%1</code>.<br>Измените свои права для записи на веб-сервер и повторите попытку.',
'SqliteReadonly'				=> 'Файл <code>%1</code> не доступен для записи.',
'SqliteCantCreateDb'			=> 'Не удалось создать файл базы данных <code>%1</code>.',
...
'CreatingIndex'					=> 'Создание индекса %1',
'CreatingTrigger'				=> 'Создание %1 триггера',
'ErrorAlreadyExists'			=> '%1 уже существует',
'ErrorCreatingTable'			=> 'Ошибка создания таблицы %1, она уже существует?',
'ErrorCreatingIndex'			=> 'Ошибка создания индекса %1 , уже существует?',
'ErrorCreatingTrigger'			=> 'Ошибка при создании %1 триггер, уже существует?',
...
'ErrorGivePrivileges'			=> 'Конфигурационный файл %1 не может быть сохранён. Нужно временно дать веб-серверу права на запись либо на каталог WackoWiki, либо на пустой файл %1<br>%2<br<br>>Не забудьте убрать эти права после установки командой:<br>%3<br><br>',
'ErrorPrivilegesInstall'		=> 'Если, почему-либо вы не можете выставить такие права, придётся скопировать текст, расположенный ниже в новый файл и загрузить его на сервер под именем %1 в каталог WackoWiki. После этого ваш сайт должен заработать. Если нет, смотрите <a href="https://wackowiki.org/doc/Doc/Русский/Инсталляция" target="_blank">WackoWiki:Doc/Русский/Инсталляция</a>',
'ErrorPrivilegesUpgrade'		=> 'После этого ваш сайт должен заработать. Если нет, смотрите <a href="https://wackowiki.org/doc/Doc/Русский/Обновление" target="_blank">WackoWiki:Doc/Русский/Обновление</a>',
'WrittenAt'						=> 'сохранён ',
