...
if ($this->user_allowed_comments())
{
	if (list ($comments, $pagination) = load_recent_comments($this, $root, (int)$max))
	{
		...
		if ($root == '' && !(int)$noxml)
		{
			echo '<span class="desc_rss_feed"><a href="'.$this->config['base_url'].'xml/comments_'.preg_replace('/[^a-zA-Z0-9]/', '', strtolower($this->config['site_name'])).'.xml"><img src="'.$this->config['theme_url'].'icon/xml.png'.'" title="'.$this->get_translation('RecentCommentsXMLTip').'" alt="XML" /></a></span>'."<br /><br />\n";
		}
