...
if (substr($this->tag, 0, strlen($this->db->forum_cluster)) == $this->db->forum_cluster)
{
	...
	// make query
	$sql =	"SELECT p.page_id, p.owner_id, p.user_id, p.tag, p.supertag, p.title, p.description, p.page_lang " .
	...
	foreach ($forums as $forum)
	{
		...
		if ($this->has_access('read', $forum['page_id']))
		{
			...
			$comments = $this->db->load_all(
				"SELECT a.page_id, a.owner_id, a.user_id, a.tag, a.supertag, a.title, a.comment_on_id, a.user_id, a.owner_id, a.created, a.page_lang, b.tag as comment_on, b.title as topic_title, b.page_lang as topic_lang, u.user_name " .
				"FROM " . $this->db->table_prefix . "page a " .
