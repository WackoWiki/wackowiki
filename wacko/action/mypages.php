...
if (($user_id = $this->get_user_id()))
{
	...
	if ($mode == 'bydate' || $bydate)
	{
		...
		if ($pages = $this->db->load_all(
			"SELECT page_id, owner_id, user_id, tag, supertag, title, created, page_lang " .
			$selector .
	...
	else if ($mode == 'bychange' || $bychange == 1)
	{
		...
		if ($pages = $this->db->load_all(
			"SELECT p.page_id, p.owner_id, p.user_id, p.tag, p.supertag, p.title, p.modified, p.page_lang " .
			$selector .
			"GROUP BY tag " .
	...
	else
	{
		...
		if (($pages = $this->db->load_all(
			"SELECT page_id, owner_id, user_id, tag, supertag, title, modified, page_lang " .
			$selector .
			"ORDER BY tag ASC " .
			$pagination['limit'], true)))
		{
