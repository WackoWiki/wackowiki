...
if (!empty($this->config['news_cluster']))
{
	...
	if (!(int)$noxml)
	{
		echo '<span class="desc_rss_feed"><a href="'.$this->config['base_url'].'xml/news_'.preg_replace('/[^a-zA-Z0-9]/', '', strtolower($this->config['site_name'])).'.xml"><img src="'.$this->config['theme_url'].'icon/xml.png'.'" title="'.$this->get_translation('RecentNewsXMLTip').'" alt="XML" /></a></span>'."\n";
	}
