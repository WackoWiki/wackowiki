...
if (!$ids)
{
	...
	if ($categories = $this->get_categories_list($lang, 1, $root))
	{
		...
		foreach ($categories as $id => $word)
		{
			...
			echo '<li class="'.( !$inline ? 'inline' : '' ).'"> '.( $list ? '<a href="'.$this->href('', '', 'category='.$id).'">' : '' ).htmlspecialchars($word['category'], ENT_COMPAT | ENT_HTML401, $this->charset).( $list ? '</a>'.' ('.(int)$word['n'].')' : '' )."";
			...
			if (isset($word['childs']) && $word['childs'] == true)
			{
				...
				{
					echo '<li class="'.( !$inline ? 'inline' : '' ).'"> '.( $list ? '<a href="'.$this->href('', '', 'category='.$id).'">' : '' ).htmlspecialchars($word['category'], ENT_COMPAT | ENT_HTML401, $this->charset).( $list ? '</a>'.' ('.(int)$word['n'].')' : '' )."</li>\n";
				}
