...
if ($pages = array_merge($pages1, $pages2))
{
	...
	if (!(int)$noxml)
	{
		echo "<a href=\"".$this->GetConfigValue("base_url")."xml/changes_".preg_replace("/[^a-zA-Z0-9]/", "", strtolower($this->GetConfigValue("wacko_name"))).".xml\"><img src=\"".$this->GetConfigValue("theme_url")."icons/xml.gif"."\" title=\"".$this->GetTranslation("RecentChangesXMLTip")."\" alt=\"XML\" /></a><br /><br />\n";
	}
	...
	foreach ($pages as $page)
	{
		...
		if ($access && ($printed[$page['tag']] != $page['date']) && ($count++ < $max))
		{
			...
			else if ($page['created'] == $page['modified'])
			{
				...
				echo "<img src=\"".$this->GetConfigValue("theme_url")."icons/add.gif"."\" title=\"".$this->GetTranslation("NewPageCreated")."\" alt=\"[new]\" /> ".''.$this->Link('/'.$page['tag'], '', $page['title'], 0, 1).' &nbsp;&nbsp;&rarr; '.$this->GetTranslation("Cluster").' '.$subtag[0].$separator.$author.'';
			}
			// new revision
			else
			{
