...
class RSS
{
		...
		if ($pages = $this->engine->LoadRecentlyChanged())
		{
			foreach ($pages as $i => $page)
			{
				if ($this->engine->config["hide_locked"])
					$access =$this->engine->HasAccess("read",$page["page_id"],"guest@wacko");
	...
	function Comments()
	{
		...
		if ( $pages = $this->engine->LoadRecentlyComment() ) {
			foreach ($pages as $i => $page) {
				if ($this->engine->config["hide_locked"]) $access =$this->engine->HasAccess("read",$page["page_id"],"guest@wacko");
				if ( $access && ($count < 30) ) {
					$count++;
					$xml .= "<item>\n";
	...
	function SiteMap()
	{
		...
		if ($pages = $this->engine->LoadAllPagesByTime())
		{
			foreach ($pages as $i => $page)
			{
				if ($this->engine->config["hide_locked"] ? $this->engine->HasAccess("read",$page["page_id"],"guest@wacko") : true)
				{
					$xml .= "<url>\n";
