	...
	else
	{
		...
		switch ($c = $files['count'])
		{
			case 0:
				...
				{
					$have_files = $this->get_translation('Files_0');
				}
				break;
			case 1:
				$have_files = $this->get_translation('Files_1');
				break;
			default:
				$have_files = str_replace('%1', $c, $this->get_translation('Files_n'));
		}
		...
		if ($have_files)
		{
			...
			echo '<h1><a href="'.$this->href('', '', 'show_files=1#header-files').'" title="'.$this->get_translation('ShowFiles').'">'.$have_files.'</a></h1>';
			echo '</header>'."\n";
			echo "</section>\n";
		}
		else
