...
if ($this->HasAccess("read")) {
	...
	if ($this->HasAccess("read", $pageA["page_id"]) && $this->HasAccess("read", $pageB["page_id"]) ) {
		...
		if ($_GET["fastdiff"] || $source == 1) {
			...
			$output .=
			str_replace("%1", "<a href=\"".$this->href("", "", ($b != -1 ? "time=".urlencode($pageA["modified"]) : ""))."\">".$this->GetTimeStringFormatted($pageA["time"])."</a>",
			str_replace("%2", "<a href=\"".$this->href("", "", ($a != -1 ? "time=".urlencode($pageB["modified"]) : ""))."\">".$this->GetTimeStringFormatted($pageB["time"])."</a>",
			...
			$out = str_replace("%1", "<a href=\"".$this->href("", "", "time=".urlencode($pageB["modified"]))."\">".$this->GetTimeStringFormatted($pageB["time"])."</a>",
			str_replace("%2", "<a href=\"".$this->href("", "", "time=".urlencode($pageA["modified"]))."\">".$this->GetTimeStringFormatted($pageA["time"])."</a>",
			str_replace("%3", $this->ComposeLinkToPage($this->tag, "", "", 0),
			"<div class=\"diffinfo\">".$this->GetTranslation("Comparison"))))."</div><br />\n<br />\n".$out;
			print $out;
