...
if ($this->has_access('create', $this->get_page_id($this->tag)))
{
	echo "<tt>".( strlen($this->tag) > 50 ? "...".substr($this->tag, -50) : $this->tag )."/</tt>".
		"<input id=\"create_subpage\" name=\"tag\" value=\"".( isset($_POST['option']) && $_POST['option'] === 1 ? htmlspecialchars($new_tag, ENT_COMPAT | ENT_HTML401, $this->charset) : '' )."\" size=\"20\" maxlength=\"255\" /> ".
...
if (substr_count($this->tag, '/') > 0)
{
	...
	if ($this->has_access('create', $this->get_page_id($parent)))
	{
		echo "<tt>".( strlen($parent) > 50 ? "...".substr($parent, -50) : $parent )."/</tt>".
			"<input id=\"create_pageparentcluster\" name=\"tag\" value=\"".( isset($_POST['option']) && $_POST['option'] === 2 ? htmlspecialchars($new_tag, ENT_COMPAT | ENT_HTML401, $this->charset) : '' )."\" size=\"20\" maxlength=\"255\" /> ".
...
echo "<input id=\"create_randompage\" name=\"tag\" value=\"".( isset($_POST['option']) && $_POST['option'] === 3 ? htmlspecialchars($new_tag, ENT_COMPAT | ENT_HTML401, $this->charset) : '' )."\" size=\"60\" maxlength=\"255\" /> ".
