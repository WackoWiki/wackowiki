...
'SecurityRisk' => 'Par sécurité, limitez maintenant le droit d&rsquo;écrire le fichier <code>config.php</code> maintenant, par exemple <code>chmod 644 config/config.php</code>.',
'RemoveSetupDirectory' => 'Par sécurité, supprimez maintenant le répertoire "setup", devenu inutile.',
'ErrorGivePrivileges' => 'Le fichier de configuration <code>config.php</code> n&rsquo;a pas pu être écrit par le serveur http dans votre répertoire WackoWiki.<br>Si possible, créez un fichier vide appelé  <code>config.php</code>, sur lequel vous attribuerez à provisoirement à tous le droit d&rsquo;écrire :<br><code>touch config/config.php</code><br><code>chmod 666 config/config.php</code><br>Nota : après enregistrement vous supprimerez ce droit, i.e. <code>chmod 644 config/config.php</code>.<br> Sinon, vous devrez copier le texte ci-dessous dans un fichier et le sauvegarder comme <code>config/config.php</code> dans le répertoire WackoWiki.<br>Ceci fait, votre site WackoWiki devrait fonctionner. Sinon, voyez  <a href="https://wackowiki.org/doc/Doc/Francophone/InstallationEtMiseAJour" target="_blank">WackoWiki:Doc/Francophone/InstallationEtMiseAJour</a>',
