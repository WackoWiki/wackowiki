...
'ErrorGivePrivileges' => 'Il file di configurazione <code>config.php</code> non si è potuto scrivere. Si dovrà rendere temporaneamente accessibile alla scrittura il tuo server e la tua directory WackoWiki, o un file vuoto col nome di <code>config.php</code> (<code>touch config.php ; chmod 666 config.php</code>; don\'t forget to remove write access again later, ie <code>chmod 644 config.php</code>). Se, per qualche motivo, non puoi farlo, dovrai copiare il testo in basso in un nuovo file e salvarlo/immetterlo come <code>config.php</code> nella directory WackoWiki. Ciò fatto, il tuo sito WackoWiki è pronto per funzionare. Altrimenti, visita <a href="https://wackowiki.org/doc/Doc/English/Installation" target="_blank">WackoWiki:Doc/English/Installation</a>',
